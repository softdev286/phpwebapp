...
// Extra Version
define('MANAGEMENT_EXTRA_VERSION', '20190930-001');
...
if (isset($_GET['key']) && check_valid_key($_GET['key']))
{
    ...
    // Check if the hosting tier should be set
    if (isset($_GET['set_hosting_tier']))
    {
        set_hosting_tier();
    }
}
...
function enable_management_extra()
{
        ...
        $simplerisk_base_url = "https://".$escaper->escapeHtml($_SERVER['HTTP_HOST']);

        // Open the database connection
        $db = db_open();

        // Insert the simplerisk_base_url if it doesn't already exist
        $stmt = $db->prepare("INSERT IGNORE INTO `settings` SET `name` = 'simplerisk_base_url', `value` = :simplerisk_base_url");
...
function check_valid_key($key) {
    ...
    if ($management_api_key && $key == $management_api_key) {
        return true;
    }
...
function enable_extras()
{
    ...
    // If the Customization Extra directory exists
    if (is_dir(realpath(__DIR__ . '/../customization')))
    {
        // Include the Customization Extra
        require_once(realpath(__DIR__ . '/../customization/index.php'));

        // DO NOT Enable the Customization Extra
        //enable_customization_extra();
    }

    // If the Advanced Search Extra directory exists
    if (is_dir(realpath(__DIR__ . '/../advanced_search')))
    {
        // Include the Advanced Search Extra
        require_once(realpath(__DIR__ . '/../advanced_search/index.php'));

        // Enable the Advanced Search Extra
        enable_advanced_search_extra();
    }

    // If the ComplianceForge SCF Extra directory exists
    if (is_dir(realpath(__DIR__ . '/../complianceforgescf')))
    {
        // Include the ComplianceForge SCF Extra
        require_once(realpath(__DIR__ . '/../complianceforgescf/index.php'));

        // Enable the ComplianceForge SCF Extra
        enable_complianceforge_scf_extra();
    }
    
    // If the Jira Extra directory exists
    if (is_dir(realpath(__DIR__ . '/../jira')))
    {
...
function disable_extras()
{
	...
	if (is_dir(realpath(__DIR__ . '/../customization')))
	{
		...
		// DO NOT Disable the Customization Extra
...
/*************************************
 * FUNCTION: MANAGEMENT EXTRA BACKUP *
 *************************************/
function management_extra_backup()
{
...
function metrics()
{
	...
	echo "<system>";
	echo "<metric>";
	echo "<name>instance_id</name>";
	echo "<value>" . $escaper->escapeHtml(get_setting("instance_id")) . "</value>";
	echo "</metric>";
	echo "<metric>";
    echo "<name>hosting_tier</name>";
    echo "<value>" . $escaper->escapeHtml(get_setting("hosting_tier")) . "</value>";
    echo "</metric>";
	echo "<metric>";
    ...
    if (is_dir(realpath(__DIR__ . '/../upgrade')))
    {
        ...
        require_once(realpath(__DIR__ . '/../upgrade/index.php'));
            ...
            echo "</metric>";
            echo "<metric>";
            echo "<name>customization_extra_version</name>";
            echo "<value>" . $escaper->escapeHtml(customization_extra_version()) . "</value>";
            echo "</metric>";
            echo "<metric>";
            echo "<name>advanced_search_extra_version</name>";
            echo "<value>" . $escaper->escapeHtml(advanced_search_extra_version()) . "</value>";
            echo "</metric>";
            echo "<metric>";
            echo "<name>complianceforge_scf_extra_version</name>";
            echo "<value>" . $escaper->escapeHtml(complianceforge_scf_extra_version()) . "</value>";
            echo "</metric>";
            echo "<metric>";
            echo "<name>jira_extra_version</name>";
            echo "<value>" . $escaper->escapeHtml(extra_current_version("jira")) . "</value>";
            echo "</metric>";
    }
...
/********************************
 * FUNCTION: UPGRADE ALL EXTRAS *
 ********************************/
function upgrade_all_extras()
{
    // If the Upgrade Extra directory exists
    if (is_dir(realpath(__DIR__ . '/../upgrade')))
    {
    ...
    if (is_dir(realpath(__DIR__ . '/../customization')))
    {   
        // Download the Customization Extra
    ...
    if (is_dir(realpath(__DIR__ . '/../advanced_search')))
    {   
        // Download the Advanced Search Extra
...
/******************************
 * FUNCTION: SET HOSTING TIER *
 ******************************/
function set_hosting_tier()
{
    $tier = $_GET['set_hosting_tier'];

    // Open the database connection
    $db = db_open();

    // If the hosting tier is trial, small, medium, or large
    if ($tier == "trial" || $tier == "small" || $tier == "medium" || $tier == "large")
    {
        // Query the database
        $stmt = $db->prepare("INSERT IGNORE INTO `settings` SET `name` = 'hosting_tier', `value` = :tier ON DUPLICATE KEY UPDATE `value` = :tier");
    ...
    {
        // Query the database
        $stmt = $db->prepare("DELETE FROM `settings` WHERE `name` = 'hosting_tier';");
        $stmt->execute();
