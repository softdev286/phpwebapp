...
{
    prevent_extra_double_submit("api", true);
    ...
    $message = "API Extra was toggled on by username \"" . $_SESSION['user'] . "\".";
...
{
    prevent_extra_double_submit("api", false);
    ...
    $message = "API Extra was toggled off by username \"" . $_SESSION['user'] . "\".";
    write_log(1000, $_SESSION['uid'], $message, 'extra');
...
/*****************************************
 * FUNCTION: CHECK KEY VALIDATION BY UID *
 *****************************************/
function is_valid_key_by_uid($key, $uid)
{
    // Get the salt
    $salt = get_api_salt();
        
    // Create the hashed api_key
    $api_key_hash = generateHash($salt, $key);
    ...
    // Check if the key and uid is matched.
    if($array)
    {
        $valid = true;
    }
...
function authenticate_key()
{
    ...
    if (isset($_GET['key']))
    {
        ...
        if (count($array) > 0)
        {
            ...
            // Query the DB for the users complete information
            $stmt = $db->prepare("SELECT value, type, name, username, lang, " . implode(',', $possible_permissions) . " FROM user WHERE value = :user_id; ");
            $stmt->bindParam(":user_id", $user_id, PDO::PARAM_INT, 11);
            $stmt->execute();
            ...
            */
            ...
            elseif (defined('LANG_DEFAULT')) {
                ...
                $_SESSION['lang'] = get_setting("default_language");
            }
